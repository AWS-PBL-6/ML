# RAG Response Guidance Layer — 구현 가이드

> AE-Sentinel 계류삭 안전관제 시스템의 **RAG 대응 안내 계층**.
> 위험판정(NORMAL/CAUTION/DANGER) 이후, Amazon Bedrock Knowledge Base(RAG)에서
> 상황별 대처방안을 검색·생성해 **Backend에 구조화 JSON으로 반환**한다.
>
> - 계정: Application SAM 스택이 배포되는 현재 AWS 계정 · 리전: **ap-northeast-2 (서울)**
...
> - 호출 구조: Backend → Response Guidance Lambda → Bedrock Knowledge Base
...
```
ML 위험판정(riskLevel)            RAG Response Guidance Layer (이 문서)
...
- 출력: 관리자용 구조화 안내(`response-guidance.v1`). 발송·저장은 Backend 책임.
...
### 지식 출처 2종
1. `approved/korean/` — 출처·사용조건·최신성 검토를 마친 한국 공식 안전 PDF.
2. `approved/port-specific/<siteId>/` — 항만사가 제공하고 안전관리자가 승인한 사내 규정 PDF.
...
agent-response-layer/
  ...
  knowledge-base/
    approved/korean/             # 한국 공식 운영 문서 메타데이터
  agent/
    query-template.md            # RetrieveAndGenerate 질의/프롬프트 템플릿
  ...
  test/
    ...
    TEST-SCENARIOS.md
  contracts/
    schemas/agent-request.v1.json        # Backend 입력 규격
    schemas/response-guidance.v1.json    # 출력 규격(계약)
    examples/agent-request.v1.json
    examples/response-guidance.v1.json
```

계약(이 저장소 내): `contracts/schemas/agent-request.v1.json`,
`contracts/schemas/response-guidance.v1.json`과 각각의 `examples/`.

---

## 3. 단일 구현 경로

Response Guidance Lambda가 `bedrock-agent-runtime.retrieve`로 승인 문서를 검색한 뒤
`bedrock-runtime.converse`로 JSON을 한 번 생성한다. 인용은 모델 문장이 아니라 Retrieve
결과의 실제 문서 메타데이터만 사용한다. 별도 Bedrock Agent·Alias·Action Group은 만들지 않는다.
...
1. 이름 예: `ae-sentinel-kb-<AWS_ACCOUNT_ID>` (전역 고유해야 함), 리전 서울.
2. 기본 보안 유지: **Block all public access = ON**, 기본 암호화(SSE-S3/KMS) 사용.
3. 버킷 안에 승인 prefix 2개를 만들고 검토 완료 PDF를 업로드:
   - `approved/korean/` ← 한국 공식 공개 문서 중 운영 승인본
   - `approved/port-specific/yard-a/` ← 해당 항만사가 제공한 승인 규정 PDF
...
```json
{ "metadataAttributes": { "approvalStatus": "approved", "scope": "port-specific", "siteId": "yard-a" } }
```
한국 일반 문서에는 `{ "metadataAttributes": { "approvalStatus": "approved", "scope": "general", "siteId": "ALL" } }`.
...
3. **데이터 소스**: Amazon S3 → 위 버킷 지정.
   - 현재 데이터 소스: `s3://ae-sentinel-kb-.../approved/korean/`
   - 항만사 승인 문서를 추가할 때는 별도 데이터 소스 또는 승인 prefix 정책을 함께 설계한다.
...
## 7. STEP 4 — Response Guidance Lambda 배포
...
2. **실행 역할**: 새 역할 생성 후, 아래 정책을 붙인다(또는 미리 IAM에서 역할 생성).
   ...
   - 권한정책: `iam/lambda-execution-policy.json` (`REPLACE_KB_ID`를 실제 값으로)
...
| `MODEL_ARN` | 현재 계정에서 사용 가능한 Bedrock inference-profile ARN |
| `REQUIRE_APPROVED_KB` | `true` |
...
## 8. STEP 5 — Backend 알림 연결

Response Guidance Lambda에는 SNS 권한과 토픽 환경변수를 주지 않는다. 기존
`ae-sentinel-app-alerts` 토픽과 DynamoDB SMS 수신자 조회는 Backend가 사용한다.

콘솔 → **SNS** → Topics → `ae-sentinel-app-alerts` → **Create subscription**.
- 이메일: Protocol=Email, Endpoint=관리자 메일 → 수신 메일에서 **Confirm** 클릭.
- SMS: Frontend 수신자 관리에서 활성·수신동의 번호를 등록한다. Backend가 직접 발송한다.
...
`ae-sentinel-response-guidance`를 **동기(RequestResponse) Invoke** 한다.
...
### 대시보드 전달
- Backend는 Lambda 반환값을 `EventRecord.responseGuidance`에 저장한다.
- `ws-risk-message.v1.data.responseGuidance`로 CAUTION/DANGER 팝업을 즉시 표시한다.
- SNS 이메일과 등록 SMS는 같은 guidance를 사용하며 `eventId`로 중복 발송을 막는다.
...
- **PoC 종료 시 정리 순서**: Knowledge Base 삭제 → OpenSearch Serverless 컬렉션 삭제
