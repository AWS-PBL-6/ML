...
계약(팀 공용): `packages/contracts/schemas/app/response-guidance.v1.json` (+ examples).
...
- 신규 산출물 계약 `packages/contracts/schemas/app/response-guidance.v1.json`은 **기존 스키마를
  건드리지 않는 additive 추가**다. 그래도 팀 공용 계약이므로
  [계약 변경 프로세스](../docs/contracts-change-process.md)에 따라 **팀 합의 후 확정**한다.
