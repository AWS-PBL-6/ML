...
def _build_query(d: dict) -> str:
    ...
    lines = [
        ...
        "판정 근거 AE 특징값:",
        f"- RMS: {g(f, 'rms')}",
        f"- 피크 진폭(peakAmplitude): {g(f, 'peakAmplitude')}",
        f"- 에너지(energy): {g(f, 'energy')}",
        f"- 우세 주파수(dominantFrequencyHz): {g(f, 'dominantFrequencyHz')} Hz",
        f"- 상승시간(riseTimeMs): {g(f, 'riseTimeMs')} ms, 지속시간(durationMs): {g(f, 'durationMs')} ms",
